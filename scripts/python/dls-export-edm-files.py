...
overview_in="-overview-src.edl"
overview_out="-overview.edl"
temp_out="-temp.edl"
flow_out="-flow.edl"
...
def main():
	...
	parser.add_option("-o", action="store_true", dest="overview", help="Exports an autofilled overview screen <dom>"+overview_out+" from src <dom>"+overview_in)
...
-----------------------------------------------------------------------------""" % xml
	...
	for name,table in data.tables:
		...
		if name==mo_table_name:
			...
			if options.overview:
				bugprint("Autofilling EDM overview screen: Motion")
				gen_edm_autofill(table,Table_handler,dom+overview_out)
			if options.temp:
		...
		if name==va_table_name:
			if options.overview:
				bugprint("Autofilling EDM overview screen: Vacuum")
				gen_edm_autofill(table,Table_handler,dom+overview_out)
			if options.vac:
				bugprint("Creating EDM vacuum screens")
				gen_edm_vac(data,out_dir)
	if options.overview:
		os.chmod(out_dir+"/"+dom+overview_out,0555)
	if options.temp:
